...
Author : Kai
Date   : 2021-01-07
Purpose: Automate mapping between units ontologies UO against QUDT

Run with QUDT:

./mapping_uo_to_qudt.py -b ../../uo_template/uo_template.csv -t ../../ontologies/qudt_export_xlsx.csv -o output_uo_to_qudt_mapping.csv -m unmapped_uo_to_qudt.csv -n unmapped_qudt_to_uo.csv -s suggestions_uo_to_qudt.csv
...
def get_args():
    ...
    parser.add_argument(
        '-t',
        '--target',
        help='Target ontology to map to (QUDT)',
        metavar='str',
        type=str,
        default='')
    ...
    parser.add_argument(
        '-o',
        '--output',
        help='Output UO to QUDT mapping',
        metavar='str',
        type=str,
        default='')
...
def main():
    ...
    header = ('UO ID', 'UO label', 'QUDT label','QUDT ID')
    ...
    for b in base_list:
        if (b['Type'] == 'class') and b['ontology ID'].startswith('UO:') and b['label'] != '':
            ...
            for t in target_list:
                ...
                if b['label'].lower() == t['LABEL'].lower():
                ...
                elif t['LABEL'].lower() in exact_syn_list:
                    for s in exact_syn_list:
                        if s == t['LABEL'].lower():
                            print(','.join(print_mapping(BaseID=b['ontology ID'],BaseLabel=b['label'],TargetID=t['ID'],TargetLabel=t['LABEL'])), file=f)

                elif t['LABEL'].lower() in related_syn_list:
    ...
    initial_mapping_qudt_ids = [o['QUDT ID'] for o in output_list]

    #Open and write suggestions file
    s_header = ('Hamm dist','UO ID', 'UO label', 'QUDT label','QUDT ID')
    f4 = open(s_file, mode='w', encoding='utf-8-sig')
    print(','.join(s_header), file=f4)

    for m in missing_list:
        for t in target_list:
            if t['ID'] not in initial_mapping_qudt_ids:
                #Hamming distance
                if (m['UO label'] != '') and (t['LABEL'] != ''):
                    d = ham_dist(m['UO label'],t['LABEL'].lower())
                    if d < 3:
    ...
    #Open and write unmapped QUDT terms
    n_header = ('QUDT label', 'QUDT ID')
    f5 = open(n_file, mode='w', encoding='utf-8-sig')
    print(','.join(n_header), file=f5)

    unmaped_QUDT = [(t['LABEL'],t['ID']) for t in target_list if t['ID'] not in initial_mapping_qudt_ids]
    unmaped_QUDT = remove_tup_duplicates(unmaped_QUDT)
    unmaped_QUDT = sort_tuples(unmaped_QUDT)

    for x in unmaped_QUDT:
